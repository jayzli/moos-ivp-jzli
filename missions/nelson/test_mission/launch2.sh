...
LOITER_POS1="270,-75"
TRANSIT_SPEED2=3.4
nsplug my_vehicle2.bhv targ_$VNAME1.bhv -f VNAME=$VNAME1     \
...
STARTPOLY=(`cat startpoint.txt`)
LOITERPOLY=(`cat loiterpoint.txt`)
...
do
   ...
   # LOITER_POS=`pickpos --poly="${LOITERPOLY[LOITER_INDEX]}" --amt=1`
    ...
    START=${STARTPOLY[$ARRAY_INDEX]}
    LOITER_POS=${LOITERPOLY[$ARRAY_INDEX]}
